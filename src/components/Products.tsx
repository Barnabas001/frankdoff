...
const icons = [laptopImg, desktopImg, printerImg, copierImg, accessImg, phoneImg]
const accentCols = ['#1d7ef5','#7c3aed','#22c55e','#F5C518','#f97316','#06b6d4']

export default function Products() {
  return (
    <section id="products" className="py-24 relative overflow-hidden" style={{ background: 'var(--bg-secondary)' }}>
      ...
      <div className="relative max-w-6xl mx-auto px-6">
          ...
          {categories.map((cat, i) => (
            ...
            >
              ...
              <div className="relative h-48 flex items-center justify-center overflow-hidden"
                   style={{ background: `linear-gradient(135deg, ${accentCols[i]}12, ${accentCols[i]}05)` }}>
                <img
                  src={icons[i]}
                  alt={cat.name}
                ...
                <span className="absolute top-3 right-3 text-[11px] font-bold px-2.5 py-1 rounded-full"
                      style={{ background: `${accentCols[i]}22`, color: accentCols[i] }}>
                  {cat.brands.length} Brands
                </span>
                {/* Arrow indicator */}
                <div className="absolute bottom-3 right-3 w-8 h-8 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-200 translate-x-2 group-hover:translate-x-0"
              ...
              <div className="p-6">
                ...
                <div className="flex items-center gap-1.5 text-sm font-bold transition-colors"
                     style={{ color: accentCols[i] }}>
                  Explore {cat.name} <span className="group-hover:translate-x-1 inline-block transition-transform">→</span>
                </div>
              </div>
            </Link>
          ))}
        ...
        {/* CTA Banner */}
        <div className="mt-10 rounded-2xl p-8 md:p-10 flex flex-col md:flex-row items-center justify-between gap-6 border"
          ...
          <a href="#contact" className="btn-primary whitespace-nowrap">Make a Request →</a>
        </div>
      </div>
    </section>
  )
